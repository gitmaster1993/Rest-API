...
public class OPExternalUpdateAPITest {

	Logger logger = Logger.getLogger("OPExternalUpdateAPITest");
	...
	public void testExternalUpdateAPI() throws IOException, InvalidFormatException {
		...
		String filePath = "src/test/resources/OPExternalUpdate.xlsx";
		...
		String drawingId = XLSXReader.readDrawingCellValue(1, 5, filePath);
		try {
			urlString = new URL(
					testBase.getPstValidationHost() + "/office/" + drawingId + "?includeCustomerEmail=true");
		...
		FileOutputStream out = new FileOutputStream(
				new File("test-output/api-test-output/OPExternalUpdateResult.xlsx"));

		for (String requestBody : listOfRows) {
			Response response = given().header(TestBase.CONTENT_TYPE, TestBase.APPLICATION_JSON)
					.header(TestBase.PSTAPIKEY, testBase.getOfficeApiKeyValue()).body(requestBody).when()
					.put(urlString);
		...
		logger.log(Level.INFO,
				() -> "Office External Update API - Success count :" + pass + ", Failure count :" + fail);
		wb.write(out);
		out.close();
